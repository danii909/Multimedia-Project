...
class VideoStabilizer:
    ...
    def _estimate_gmv_optical_flow(self, prev_gray: np.ndarray, curr_gray: np.ndarray) -> Tuple[float, float, float]:
        """Stima GMV sub-pixel usando LK optical flow + RANSAC affine parziale (con rotazione)."""
        ...
        # Estrai traslazione e rotazione dalla matrice affine
        tx = float(M[0, 2])
        ty = float(M[1, 2])
    ...
    def stabilize_video(self, 
        ...
        self._first_pass(cap, total_frames, show_progress)
        ...
        self._second_pass(cap, out, total_frames, show_progress)
    ...
    def _first_pass(self, cap, total_frames: int, show_progress: bool):
        """Prima passata: stima movimento e costruisci traiettoria."""
        import time
    ...
    def _second_pass(self, cap, out, total_frames: int, show_progress: bool):
